            ...
            <div className="pfp-viewer-overlay" onClick={() => setViewerIdx(null)}>
              <div className="pfp-viewer-wrap" onClick={e => e.stopPropagation()}
                ...
                onTouchEnd={e => {
                  ...
                  if (delta < 0 && viewerIdx < photos.length - 1) setViewerIdx(viewerIdx + 1)
                  if (delta > 0 && viewerIdx > 0) setViewerIdx(viewerIdx - 1)
                }}
              >
    ...
    <div className="pfp-page">
      <div className="pfp-topbar">
        ...
        <div className="pfp-auth-zone">
          {isClubOwner ? (
            ...
            <>
              <span className="pfp-auth-name">Hi, {publicAccount.display_name?.split(' ')[0] || publicAccount.email?.split('@')[0]}</span>
              <button className={`pfp-saved-toggle ${showSaved ? 'pfp-saved-toggle--active' : ''}`} onClick={() => { setShowSaved(s => !s); setPanelOpen(true) }} title="Saved clubs">
                <svg width="14" height="14" viewBox="0 0 24 24" fill={favIds.size > 0 ? '#e24b4a' : 'none'} stroke={favIds.size > 0 ? '#e24b4a' : 'currentColor'} strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                {favIds.size > 0 && <span className="pfp-saved-badge">{favIds.size}</span>}
              </button>
      ...
      <div className={`pfp-body ${showDisclaimer ? 'pfp-body--blurred' : ''}`}>
        ...
        <div className="pfp-overlay">
          ...
          {/* Saved clubs panel */}
          {showSaved && publicAccount && (
            <div className={`pfp-panel pfp-panel--open`}>
              <div className="pfp-panel-header" onClick={() => { setShowSaved(false); setPanelOpen(true) }}>
                <span className="pfp-panel-count">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="#e24b4a" stroke="#e24b4a" strokeWidth="2" style={{marginRight:5,verticalAlign:-1}}><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                  {savedClubs.length} saved club{savedClubs.length !== 1 ? 's' : ''}
                </span>
                <button className="pfp-saved-back" onClick={e => { e.stopPropagation(); setShowSaved(false) }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
                </button>
              </div>
              <div className="pfp-panel-list">
                {savedClubs.length === 0 ? (
                  <div className="pfp-saved-empty">
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="1.5"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                    <p>No saved clubs yet</p>
                    <p className="pfp-saved-empty-sub">Tap the heart on any club to save it here.</p>
                  </div>
                ) : savedClubs.map(club => (
                  <div key={club.id} id={`pfp-card-${club.id}`}>
                    <ClubCard
                      ...
                      onToggleFav={() => toggleFavorite(club.id)}
